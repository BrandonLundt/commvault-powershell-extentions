...
<#
    ...
    .DESCRIPTION
	    Removes VM from collection\Subclient
	    Require minimum powershell version 3.0
#>

<#
    .SYNOPSIS
        Removes VM from subclient\Collection content.

    .PARAMETER 
        -ClientName    -> Name of the client in subclient exists
        -SubclientName -> Subclient name to which content should be rmeoved form. If this parametrer not passed, commandlet assumes default
        -EntityType    -> EntityType can be VM, Host or Cluster etc
        -Entity        -> VirtualMachine Name, Datastore name etc
        
    
    .OUTPUT
        Removes content from subclient.

    .Usage
        Example1: Remove-VM -ClientName <myClient>  -SubclientName <testSubclient> -EntityType <VM> -Entity <myVM>
        Example2: Remove-VM -ClientName <myClient>  -EntityType <VM> -Entity <VM1,VM2,VM3>
...
<#
     .Example
        Remove-VM -ClientName <myClient>  -SubclientName <testSubclient> -EntityType <VM> -Entity <myVM>
     .Example
        Remove-VM -ClientName <myClient>  -EntityType <VM> -Entity <VM1,VM2,VM3>
#>
[CmdletBinding()]
...
[String] $Entity
    ...
    Process{
        ...
        try{
            ...
            if($prop.response)
            {
                $outString = "Removed the entity "+ $Entity
                return $outString
            }
